...
const AddRoom = () => {
  ...
  return (
    <form >
      <Title align='left' font='outfit' title='Add Room' subTitle='Fill out the form below to add a new room, including its details, capacity, features, and availability schedule.' />
      ...
      <div className='flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm'>
        {
          Object.keys(inputs.amenities).map((amenity,index)=>(
            <div key={index}>
              <input type="checkbox" id={`amenties${index+1}`} checked={inputs.amenities[amenity]} onChange={()=>setInputs({...inputs,amenities:{...inputs.amenities,[amenity]:!inputs.amenities[amenity]}})}  />
              <label htmlFor={`ameities${index+1}`}>{amenity}</label>
            </div>
          ))
        }
      </div>
